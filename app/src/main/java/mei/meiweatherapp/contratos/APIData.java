...
public class APIData {
    public static class Accuweather
    {
        ...
        private static String METHOD_GeoPosition  = "/locations/v1/cities/geoposition/search?apikey={APIKEY}&q={LATITUDE}%2C{LONGITUDE}&language=pt-ptt&toplevel={MAISPROXIMA}";
        private static String METHOD_CurrentConditions = "/currentconditions/v1/{LOCATIONKEY}?apikey={APIKEY}&details={DETAILS}";
        ...
        {
            return METHOD_GeoPosition.replace("{APIKEY}", APIKEY).replace("{LOCATIONKEY}",locationKey).replace("{DETAILS}",details.toString());
        }
    }
}
